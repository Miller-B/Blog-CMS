    ...
    <body>
        <div class="wrapper">
            ...
            <div id="content">
                ...
                <div class="line"></div>
                <c:forEach items="${avm.spvms}" var="currentApvm" varStatus="loop">
                    <div class="apost">
                        <h3>${currentApvm.post.postTitle}</h3>
                        <p>Posted by: ${currentApvm.post.user.userName}</p>
                        <p>Category: ${currentApvm.post.category.categoryName}</p>                    
                        <p>${currentApvm.post.postContent}</p>              
                        <p>${currentApvm.post.startDate}</p>
                        <c:forEach items="${currentApvm.hashtags}" var="currentHashtag" varStatus="loop">
                            <a href="${pageContext.request.contextPath}/AdminManagePosts/ByHashtag?hashtagId=${currentHashtag.hashtagId}">
                                <c:out value="${currentHashtag.tagName}"/>
                            </a>
                        </c:forEach>
                        <br>
                        <br>
                        <a href="${pageContext.request.contextPath}/UpdatePostPage?postId=${currentApvm.post.postId}">Edit</a><text> | </text> 
                        <a href="${pageContext.request.contextPath}/DeletePostPage?postId=${currentApvm.post.postId}">Delete</a>                          
                    </div>
                    <div class="spacingDiv"></div>
                    <div class="line"></div>                    
                </c:forEach>
